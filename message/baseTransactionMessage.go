...
func (a TransactionsMessage) GetTransactionsFromBytes(sigName, sigName2 string) (map[[2]byte][]transactionsDefinition.Transaction, error) {
	...
	for _, topic := range validTopics {
		if _, ok := a.TransactionsBytes[topic]; ok {
			for _, tb := range a.TransactionsBytes[topic] {
				...
				if at.Verify(sigName, sigName2) || topic == tcpip.NonceTopic || topic == tcpip.SelfNonceTopic {
